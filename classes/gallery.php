...
class gallery extends base {
    ...
    static protected $table = 'uploadavatar_gallery';
    ...
    public function copy($targetid) {
        global $DB, $USER;
        if (!$DB->record_exists('uploadavatar', array('id' => $targetid))) {
            return false;
        }
        ...
        // Update thumbnail.
        $DB->set_field('uploadavatar_gallery', 'thumbnail', $thumbnail, array('id' => $newgallery->id));
    ...
    public static function create(\stdClass $data) {
        ...
        if (empty($data->noevent)) {
            $event = \mod_uploadavatar\event\gallery_created::create($params);
            $event->add_record_snapshot('uploadavatar_gallery', $result->get_record());
            $event->trigger();
        }
    ...
    public function delete($options = array()) {
        ...
        $event = \mod_uploadavatar\event\gallery_deleted::create($params);
        $event->add_record_snapshot('uploadavatar_gallery', $this->record);
        $event->trigger();
    ...
    public function download_items(array $list = array()) {
        ...
        $fs = get_file_storage();
        $filename = clean_filename('uploadavatar-export-'.$this->record->name.'.zip');

        $files = $fs->get_area_files($this->get_collection()->context->id, 'mod_uploadavatar', 'item', false, 'id', false);
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'uploadavatar_');
    ...
    public function get_items() {
        ...
        $sql = "SELECT i.*, u.firstname, u.lastname, u.username, gr.name as groupname
                ...
                JOIN {uploadavatar_gallery} g ON g.id = i.galleryid
        ...
        $files = $fs->get_area_files($this->get_collection()->context->id, 'mod_uploadavatar', 'thumbnail', false, 'id', false);
        foreach ($files as $file) {
            $filelist[$file->get_itemid()]['thumbnail'] = $file;
        }
        $files = $fs->get_area_files($this->get_collection()->context->id, 'mod_uploadavatar', 'lowres', false, 'id', false);
        foreach ($files as $file) {
    ...
    public function has_items() {
        ...
        $result = $DB->count_records('uploadavatar_item', array('galleryid' => $this->record->id));
        return !empty($result);
    }
    ...
    public function get_thumbnail() {
        ...
        if (empty($this->record->thumbnail) ||
            (!$record = $DB->get_record('uploadavatar_item', array('id' => $this->record->thumbnail)))) {
            ...
            if ($this->record->thumbnail != $thumbid) {
                $DB->set_field('uploadavatar_gallery', 'thumbnail', $thumbid, array('id' => $this->id));
            }
        }
        if (!$record) {
            return $OUTPUT->image_url('galleryicon', 'uploadavatar')->out(false);
            return null;
        }
    ...
    public function moral_rights_asserted() {
        ...
        $count = $DB->count_records('uploadavatar_item', array('galleryid' => $this->record->id, 'moralrights' => 0));
        return $count == 0;
    }

    /**
     * Checks if this gallery has been submitted for grading in the
     * assignsubmission_uploadavatar plugin.
    ...
    public function update_sortorder($data) {
        ...
        $items = $DB->get_records('uploadavatar_item', array('galleryid' => $this->record->id), '', 'id, sortorder');
        ...
        foreach ($flipped as $id => $order) {
            $DB->set_field('uploadavatar_item', 'sortorder', $order, array('id' => $id));
        }
        return true;
    }
    ...
    public function user_can_view($userid = null) {
        ...
        if (!empty($this->record->visibleinstructor) && has_capability('mod/uploadavatar:manage', $coll->context)) {
        ...
        if (has_capability('mod/uploadavatar:viewall', $coll->context)) {
            return true;
        }
        ...
        if (!empty($this->record->visibleother) && !has_capability('mod/uploadavatar:viewall', $coll->context)) {
