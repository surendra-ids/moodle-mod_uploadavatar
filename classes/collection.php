...
class collection extends base {
    ...
    static protected $table = 'uploadavatar';
    ...
    public function __construct($recordorid, $options = array()) {
        ...
        $this->cm = get_coursemodule_from_instance('uploadavatar', $this->record->id);
    ...
    public static function create(\stdClass $data) {
        ...
        $data->id = uploadavatar_add_instance($data);
    ...
    public function delete($options = array()) {
        ...
        if (!empty($params['context'])) {
            $event = \mod_uploadavatar\event\collection_deleted::create($params);
            $event->add_record_snapshot(static::$table, $this->get_record());
            $event->trigger();
        }

        $sql = "DELETE FROM {uploadavatar_item}
                WHERE galleryid IN (
                    SELECT id
        ...
        $DB->execute($sql, array($this->id));
        $DB->delete_records('uploadavatar_gallery', array('instanceid' => $this->id));
        $DB->delete_records('uploadavatar', array('id' => $this->id));
    ...
    public function get_my_galleries($userid = null) {
        ...
        if ($records = $DB->get_records_select('uploadavatar_gallery', $select, $params)) {
    ...
    public function get_submitted_galleries() {
        ...
        if (!self::is_assignsubmission_uploadavatar_installed()) {
            return false;
        }
        ...
        $sql = "SELECT asub.id, asm.galleryid, asub.status, asub.assignment, asub.userid
                ...
                WHERE apc.plugin = 'uploadavatar' AND apc.subtype = 'assignsubmission' AND apc.name = 'uploadavatar'
    ...
    public function get_visible_galleries() {
        ...
        $viewall = has_capability('mod/uploadavatar:viewall', $this->context);
    ...
    public function count_galleries() {
        global $DB;
        return $DB->count_records('uploadavatar_gallery', array('instanceid' => $this->record->id));
    }
    ...
    public function get_galleries($filterbymode = false) {
        ...
        $records = $DB->get_records('uploadavatar_gallery', $params);
        foreach ($records as $record) {
    ...
    public static function get_public_galleries_by_contextid($contextid, $prefix = true) {
        ...
        $collections = get_all_instances_in_course('uploadavatar', $course);
        ...
        $sql = "SELECT g.*,
                ...
                JOIN {uploadavatar} mg on (mg.id = g.instanceid)
    ...
    public function has_submitted() {
        ...
        if (!$this->is_assessable() || !self::is_assignsubmission_uploadavatar_installed()) {
            return false;
        }
    ...
    public function get_linked_assignid() {
        ...
        $sql = "SELECT cm.id
                ...
                WHERE apc.plugin = 'uploadavatar' AND apc.name = 'uploadavatar'
     ...
     * Users with the mod/uploadavatar:manage cap can always edit the collection.
     *
     * @access public
     * @return bool true if read only for this user.
     */
    public function is_read_only() {
        if (has_capability('mod/uploadavatar:manage', $this->context)) {
            return false;
        }
    ...
    public function user_can_remove($userid = null) {
        ...
        return $theboxowner || has_capability('mod/uploadavatar:manage', $this->get_context(), $userid);
    }
    ...
    public function user_can_add_children($userid = null) {
        ...
        $manager = has_capability('mod/uploadavatar:manage', $this->context, $userid);
        if ($max != 0 && !$manager) {
            if ($this->options['groupmode'] != NOGROUPS) {
