...
class item extends base {
    ...
    protected $context;
    protected $gallery;
    static protected $table = 'uploadavatar_item';
    ...
    public static function create(\stdClass $data) {
        ...
        if (!empty($data->thumbnail)) {
            $sql = "UPDATE {uploadavatar_gallery}
                    SET thumbnail = :item
                    WHERE id = :galleryid";
        ...
        $event = \mod_uploadavatar\event\item_created::create($params);
        $event->add_record_snapshot('uploadavatar_item', $result->get_record());
        $event->trigger();
    ...
    public static function create_from_archive(gallery $gallery, \stored_file $storedfile, $formdata = array()) {
        ...
        $count = $DB->count_records('uploadavatar_item', array('galleryid' => $gallery->id));
        ...
        $fs->delete_area_files($context->id, 'mod_uploadavatar', 'unpacktemp', 0);
        $storedfile->extract_to_storage($packer, $context->id, 'mod_uploadavatar', 'unpacktemp', 0, '/');
        $itemfiles = $fs->get_area_files($context->id, 'mod_uploadavatar', 'unpacktemp', 0);
        $storedfile->delete();
        ...
        foreach ($itemfiles as $storedfile) {
            ...
            $fileinfo = array(
                ...
                'component'     => 'mod_uploadavatar',
                'filearea'      => 'item',
                'itemid'        => $item->id,
                'filepath'      => '/',
                'filename'      => $filename
            );
            if (!$fs->get_file($context->id, 'mod_uploadavatar', 'item', $item->id, '/', $filename)) {
    ...
    public function delete($options = array()) {
        ...
        $event = \mod_uploadavatar\event\item_deleted::create($params);
        $event->add_record_snapshot('uploadavatar_item', $this->record);
        $event->trigger();

        $fs = get_file_storage();
        $fs->delete_area_files($this->get_context()->id, 'mod_uploadavatar', 'item', $this->record->id);
        $fs->delete_area_files($this->get_context()->id, 'mod_uploadavatar', 'lowres', $this->record->id);
        $fs->delete_area_files($this->get_context()->id, 'mod_uploadavatar', 'thumbnail', $this->record->id);

        // $DB->delete_records('uploadavatar_userfeedback', array('itemid' => $this->record->id));
    ...
    public static function delete_all_by_gallery($galleryid) {
        ...
        // Bulk delete files.
        if ($itemids = $DB->get_records('uploadavatar_item', array('galleryid' => $galleryid), '', 'id')) {
            $fs = get_file_storage();

            list($insql, $params) = $DB->get_in_or_equal(array_keys($itemids), SQL_PARAMS_NAMED, 'mgi');
            $gallery = new gallery($galleryid);
            $cm = get_coursemodule_from_instance('uploadavatar', $gallery->instanceid);
            $context = \context_module::instance($cm->id);
            $fs->delete_area_files_select($context->id, 'mod_uploadavatar', 'item', $insql, $params);
            $fs->delete_area_files_select($context->id, 'mod_uploadavatar', 'lowres', $insql, $params);
            $fs->delete_area_files_select($context->id, 'mod_uploadavatar', 'thumbnail', $insql, $params);
        }

        // Bulk delete userfeedback.
        $sql = "DELETE FROM {uploadavatar_userfeedback}
                WHERE itemid IN (
                    SELECT id
                    FROM {uploadavatar_item}
                    WHERE galleryid = :galleryid
                )";
        $DB->execute($sql, array('galleryid' => $galleryid));
        $DB->delete_records('uploadavatar_item', array('galleryid' => $galleryid));
        return true;
    }
    ...
    public function generate_image_by_type($type = 'thumbnail', $force = false, \stored_file $file = null) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($this->get_context()->id, 'mod_uploadavatar', $type, $this->record->id);
    ...
    public function get_embed_url() {
        ...
        if ($id = $this->get_youtube_videoid()) {
            ...
            if (!empty($this->objectid)) {
                ...
                $embed = \moodle_url::make_pluginfile_url($this->get_context()->id, 'mod_uploadavatar', 'item', $this->record->id, '/', $file->get_filename());
            }
        }
        return $embed;
    }
    ...
    public function get_file($thumbnail = false) {
        ...
        $files = $fs->get_area_files($this->get_context()->id, 'mod_uploadavatar', $filearea, $this->record->id, 'id', false);
    ...
    private function get_stored_file_by_type($type = 'item') {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_uploadavatar', $type, $this->record->id, 'id', false);
    ...
    public function get_image_url($preview = false) {
        ...
        $path = \moodle_url::make_pluginfile_url($this->get_context()->id, 'mod_uploadavatar', $type, $this->record->id, '/', $file->get_filename());
    ...
    public function get_image_url_by_type($type = 'item') {
        ...
        if ($this->record->externalurl) {
            ...
            if (preg_match('#^'.$CFG->wwwroot.'/repository/([a-z][a-z0-9]*)/#', $url, $matches) && !empty($matches)) {
                ...
                if (method_exists($class, "get_uploadavatar_link")) {
                    return $class::get_uploadavatar_link($this->record->externalurl, $type);
                }
        ...
        $path = \moodle_url::make_pluginfile_url($this->get_context()->id, 'mod_uploadavatar', $urltype, $this->record->id, '/', $file->get_filename());
    ...
    public function get_like_count() {
        ...
        $count = $DB->count_records_select('uploadavatar_userfeedback', $select, array('itemid' => $this->record->id));
    ...
    public function get_socialinfo() {
        ...
        if ($this->gallery->can_like()) {
            $info->likes = $DB->count_records('uploadavatar_userfeedback', array('itemid' => $this->record->id, 'liked' => 1));
    ...
    public function get_userfeedback() {
        global $DB, $USER;
        return $DB->get_record('uploadavatar_userfeedback', array('itemid' => $this->record->id, 'userid' => $USER->id));
    }
    ...
    public function like() {
        ...
        if (!has_capability('mod/uploadavatar:like', $this->get_context())) {
            return false;
        }

        if ($fb = $this->get_userfeedback()) {
            $fb->liked = 1;
            $DB->update_record('uploadavatar_userfeedback', $fb);
        } else {
            $fb = (object) array(
                'itemid' => $this->record->id,
                'userid' => $USER->id,
            ...
            $DB->insert_record('uploadavatar_userfeedback', $fb);
        }
    ...
    public function user_can_edit($userid = null) {
        ...
        if ($userid == $this->record->userid || has_capability('mod/uploadavatar:manage', $this->get_context(), $userid)) {
            return true;
        }
    ...
    public function user_can_remove($userid = null) {
        ...
        return has_capability('mod/uploadavatar:manage', $this->get_context(), $userid);
    }
